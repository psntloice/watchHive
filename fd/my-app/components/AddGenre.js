...
const GenreForm = () => {
  ...
  const rehandleChange = (e) => {
    const { name, value } = e.target;
    resetNewGenre (prevState => ({ ...prevState, [name]: value }));
  };
  ...
  const addGenre  = async (payload) => {
    try {
      const data = await post_call_module(payload,"genres");
  ...
  const removeGenre = async (id) => {
    try {
      const data = await delete_call_module("genres",id);
      refetch();
      console.log(data); // Log the authors data
      return data;
    } catch (error) {
  ...
  const updateGenre = async (id, payload) => {
    try {
      ...
      console.log(genre);
      const data = await put_call_module(author,"genres",id);
    } catch (error) {
      console.error('Error deleting genres:', error);
    }
  };
  ...
  const handleReSubmit = async (e, tid) => {
    e.preventDefault();
    if (tid) {
      await updateGenre(tid,renewGenre);
      resetNewGenre({ rename: '', redescription: '' });
    refetch();
    ...
    {data.length > 0 ? ( data.map((movie) => (
        <div key={movie.id}   onMouseEnter={() => HoverDiv(movie.id)}
          ...
          <div class="flex w-full gap-4" >
            ...
            <button onClick={() => removeGenre(movie.id)} type="button" className="text-white font-small rounded-lg text-xs font-thin w-min h-max" style={{ background: 'linear-gradient(180deg, #5c6db3 , #232c31, #5c6db3)' }}>
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" class="size-4">
          ...
          >
            ...
            <Input
              type="text"
              name="redescription"
              value={renewGenre.redescription}
              onChange={rehandleChange}
              isRequired
              color="primary"
